---
title: "P50 x rooting depth x aridity x WTD x deciduousness"
output:
  html_document:
    toc: true
    toc_float: true
...
m <- readRDS("results/models/arid_wtd_traits_pheno.rds")
...
```{r}
post <- rmvn(2000, coef(m)[1:40], vcov(m)[1:40,1:40]) %>%
  as_tibble() %>%
  setNames(names(coef(m))[1:40]) %>%
  janitor::clean_names()
...
X <- model.matrix(y ~ decid*(arid*wtd*p50 + p502), data = grid) %>% 
  as_tibble() %>% 
  janitor::clean_names() %>%
  as.matrix()
...
df <- grid %>%
  mutate(arid = label_arid(arid),
         ...
         decid = ifelse(decid == 1, "deciduous", "evergreen"))

ggplot(df, aes(x = p50, y = est)) + 
  geom_path(aes(color = decid)) + 
  geom_ribbon(aes(ymin = lo, ymax = hi, fill = decid), alpha = 0.2) +
...
X <- model.matrix(y ~ decid*(arid*wtd*rd + rd2), data = grid) %>% 
  as_tibble() %>% 
  janitor::clean_names() %>%
  as.matrix()
...
X <- model.matrix(y ~ decid*(arid*wtd*p50*rd + arid2 + wtd2 + p502 + rd2), data = grid) %>% 
  as_tibble() %>% 
  janitor::clean_names() %>%
  as.matrix()
...
X <- model.matrix(y ~ decid*(arid*p50*rd + arid2), data = grid) %>% 
  as_tibble() %>% 
  janitor::clean_names() %>%
  as.matrix()
...
X <- model.matrix(y ~ decid*(wtd*p50*rd + wtd2), data = grid) %>% 
  as_tibble() %>% 
  janitor::clean_names() %>%
  as.matrix()
